...
public class MedicResources {
	...
	public Response log_medic(String json_info){
		...
		System.out.println(medic_json.getString("name"));
	...
	@Path("/chat")
	@Consumes(MediaType.APPLICATION_JSON)
	public Response msg_chat(String json_info){
		
		JSONObject msg_chat = new JSONObject(json_info);
		String msg = msg_chat.getString("msg");
		System.out.println(msg_chat.getString("msg"));
		
        Chat.getList().add(msg.toString());
	...
	public Response get_chat_list(){
		return Response.ok(Finder.get_chat_msg().toString()).build();
	}
